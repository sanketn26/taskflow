...
The worker connects only to the configured local Unix socket. It performs `PULL`, receives `TASK`, starts a heartbeat tied to `lease_id`, and uses agent RPCs to read/write objects. One connection may multiplex requests only if request correlation is explicit; otherwise use a dedicated control connection and object-transfer connection.
...
The agent starts workers with configured import modules/entry points loaded before polling. Inline functions use the reserved `__inline__` identity and are accepted only when `tasks.allow_inline_functions` is enabled; this mode is explicitly trusted-code development compatibility.

## Execution Contract

1. Decode and validate `LeasedTask`.
2. Resolve the registered callable.
3. Fetch `args` when it is an `ObjectRef`; verify size and SHA-256.
4. Deserialize `{args, kwargs}` using the declared codec.
...
5. Worker-manager integration, SQLite/filesystem E2E, and chaos scenarios.
