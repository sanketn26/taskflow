...
This file is a non-normative decision summary. The complete interfaces, records, ordering rules, backend behavior, configuration, tests, and exit gates are embedded in Phases 1–7. Implementers do not need this document.
...
The zero-infrastructure defaults are SQLite and the local filesystem. Memory implementations are for tests and explicitly ephemeral development. PostgreSQL/S3 adapters (Phase 6) must pass the same conformance contracts as the Phase 2 defaults before being advertised.
...
- Phase 6: PostgreSQL state store and S3 object store adapters, conformance-tested against the Phase 2 interfaces.
- Phase 7: transactional terminal-event outbox.
